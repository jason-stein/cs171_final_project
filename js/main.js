...
function createVis(error, perDayData, metaData){
	...
	d3.select("#timestart").text(dateFormatter(extent[0]) + " - " );
	d3.select("#timestop").text(dateFormatter(extent[1]));
	...
	$(myEventHandler).bind("selectionChanged", function(event, rangeStart, rangeEnd){
    	...
    	d3.select("#timestart").text(dateFormatter(rangeStart)  + " - " );
		d3.select("#timestop").text(dateFormatter(rangeEnd));
	});
